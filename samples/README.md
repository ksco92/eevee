...
| `analytics/customers.json` | `postgres_18` | A broad Postgres type sampler (exact, parameterized, array `T[]`, multiword like `timestamp with time zone`); a **composite primary key** that includes the `range` partition column (as Postgres requires); secondary `indexes` (a unique `btree` with `include`, and a `gin` index); a `uniqueConstraints` and a `checkConstraints` entry; cross-schema `dependsOn` + FK to a primary key (no warning, `allowNulls: false`). |
