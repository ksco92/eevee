...
| `analytics/customers.json` | `postgres_18` | A broad Postgres type sampler (exact, parameterized, array `T[]`, multiword like `timestamp with time zone`); a **composite primary key** that includes the `range` partition column (as Postgres requires); secondary `indexes` (a unique `btree` with `include`, and a `gin` index); a `uniqueConstraints` and a `checkConstraints` entry; a **stored generated column** (`lifetime_value_cents`); cross-schema `dependsOn` + FK to a primary key (no warning, `allowNulls: false`). |
...
| `analytics/order_lines.json` | `hive_parquet` | A **composite primary key**; Hive `bucketing` (`CLUSTERED BY … INTO 16 BUCKETS SORTED BY`); multiple cross-schema `dependsOn`; an FK to a PK and an FK to a **non-PK** (the `FK_SOURCE_IS_KEY` warning), with `allowNulls` both ways; a Hive partition on a non-raw table. |
