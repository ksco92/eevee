...
| `analytics/customers.json` | `postgres_18` | A broad Postgres type sampler (exact, parameterized, array `T[]`, multiword like `timestamp with time zone`); Postgres declarative `range` partitioning; secondary `indexes` (a unique `btree` with `include`, and a `gin` index); cross-schema `dependsOn` + FK to a primary key (no warning, `allowNulls: false`). |
