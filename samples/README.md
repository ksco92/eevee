...
| `raw/org_chart.json` | `postgres_18` | A **self-referential foreign key** (`manager_id` → its own `emp_id`); a Postgres type sampler. |
| `analytics/customers.json` | `postgres_18` | A broad Postgres type sampler (exact, parameterized, array `T[]`, multiword like `timestamp with time zone`); cross-schema `dependsOn` + FK to a primary key (no warning, `allowNulls: false`). |
...
Every file also carries `$schema` and `specVersion`. Postgres tables have no `partitions` (rejected in
v0). Cross-schema references (`dependsOn` and foreign keys spanning `raw` and `analytics`) appear
throughout.
